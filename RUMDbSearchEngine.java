...
public class RUMDbSearchEngine {
	...
	public void insertMoviesIntoHashTable ( String inputFile ) {
		ArrayList<ArrayList<String>> a = readInputFile(inputFile);
		for( ArrayList<String> e : a){
			for(int i = 1; i < e.size(); i++){
				if(isWord(e.get(i)) != null){
					insertWordLocation(isWord(e.get(i)), new Location(e.get(0), i));
				}
			}
		}
	...
	public void insertWordLocation (String word, Location loc) {
		if(getWordOccurrence(word) == null){
			wordCount++;
			WordOccurrence a = hashTable[hashFunction(word)];
			if(a == null){
				hashTable[hashFunction(word)] = new WordOccurrence(word);
				hashTable[hashFunction(word)].addOccurrence(loc);
			}else{
			WordOccurrence b = new WordOccurrence(word);
			b.addOccurrence(loc);
			b.next = a;
			hashTable[hashFunction(word)] = b;
		}
	...
	private void rehash (int newHashSize){
		WordOccurrence[] a = new WordOccurrence[newHashSize];
		int old = hashSize;
		hashSize = newHashSize;
		for(int b = 0; b < old; b++){
			for(WordOccurrence i = hashTable[b]; i != null; i=i.next){
				WordOccurrence m = new WordOccurrence(i.getWord());
				for(int l = 0; l <i.getLocations().size();l++){
					m.addOccurrence(i.getLocations().get(l));
				}
					m.next = a[hashFunction(i.getWord())];
					a[hashFunction(i.getWord())] = m;
				}
			}
	...
	public WordOccurrence getWordOccurrence (String word) {
		...
		if(pointer == null)
			return null;
		while(pointer.next != null && !pointer.getWord().equals(word)){
	...
	public ArrayList<MovieSearchResult> createMovieSearchResult (String wordA, String wordB) {
		...
		ArrayList<Location> a = getWordOccurrence(wordA).getLocations();
		ArrayList<Location> b = getWordOccurrence(wordB).getLocations();
		

		ArrayList<MovieSearchResult> r = new ArrayList<MovieSearchResult>();

		for(Location e : a){
			int h = doesExsist(r,e.getTitle());
			if( h== -1){
			MovieSearchResult m = new MovieSearchResult(e.getTitle());
			m.addOccurrenceA(e.getPosition());
			r.add(m);
			}else{
				r.get(h).addOccurrenceA(e.getPosition());
			}
		}
		for(Location be : b){
			int h2 = doesExsist(r,be.getTitle());
			if( h2== -1){
			MovieSearchResult m2 = new MovieSearchResult(be.getTitle());
			m2.addOccurrenceB(be.getPosition());
			r.add(m2);
			}else{
				r.get(h2).addOccurrenceB(be.getPosition());
			}
		}
	...
	public int doesExsist(ArrayList<MovieSearchResult> list, String title){
	...
	public void calculateMinDistance(MovieSearchResult msr){
		...
		for(int i = 0; i < a.size(); i++){
			for(int j = 0; j< b.size(); j++){
				int l = (int)Math.abs(a.get(i)-b.get(j));
				if(l< min || min == -1){
					min = l;
				}
			}
	...
	public ArrayList<MovieSearchResult> topTenSearch(String wordA, String wordB){
		...
		ArrayList<MovieSearchResult> a = createMovieSearchResult(wordA, wordB);
		ArrayList<MovieSearchResult> b = new ArrayList<MovieSearchResult>();
		if(a == null){
			return null;
		}
		for(MovieSearchResult e : a){
			calculateMinDistance(e);
		}
		Collections.sort(a);
		
		int c = 0;
		while(c < a.size() && c < 10 && a.get(c).getMinDistance() != -1){
			b.add(a.get(c));
			c++;
		}
		return b;
